...
#                         st.download_button("📎 PDF İndir", f, file_name=os.path.basename(pdf), key=f"pdf_{pid}")
#                 else:
...
#             try:
#                 # Model eğitimi sırasında kullanılan sınıfları kontrol et
#                 encoder_classes = list(encoders[col].classes_)
                
#                 # Bilinmeyen değerleri tespit et
...
#                     # Bilinmeyen değerleri ilk sınıfla değiştir
#                     df[col] = df[col].replace(list(unknown_values), encoder_classes[0])
                
#                 # Encode işlemini gerçekleştir
...
#     # Kategorik değişkenler - modelde kullanılan etiketlerle uyumlu hale getir
#     cat_vars = [
...
#     # Encoder'ların beklediği format - genellikle sayısal
#     bin_opts = [0, 1]  # "Yok", "Var" yerine 0, 1 kullan
#     bin_labels = ["Yok", "Var"]  # Görsel amaçlı etiketler
    
#     sex_opts = [0, 1]  # "Kız", "Erkek" yerine 0, 1 kullan  
#     sex_labels = ["Kız", "Erkek"]  # Görsel amaçlı etiketler
...
#         # Cinsiyet seçimi - sayısal değer olarak kaydet
#         sex_selection = st.selectbox("Cinsiyet", sex_labels)
#         input_data["Cinsiyet"] = sex_opts[sex_labels.index(sex_selection)]

#         with st.expander("Gelişimsel ve Yapısal Bulgular"):
#             cols = st.columns(3)
#             for i, v in enumerate(cat_vars):
#                 with cols[i % 3]:
#                     # Kullanıcıya etiketleri göster ama sayısal değeri kaydet
#                     selection = st.selectbox(v, bin_labels, key=f"cat_{v}")
#                     input_data[v] = bin_opts[bin_labels.index(selection)]
...
#                 # Kategorik özellikleri güvenli encode et
#                 df = safe_encode_categorical(df, encoders)

#                 # Modeldeki feature sırasına göre eksikleri sıfırla tamamla
...
#         register_screen()
# else:
#     menu = st.sidebar.selectbox("Menü", ["Tahmin Aracı", "Çıkış"])
